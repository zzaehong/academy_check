...
관리자 선생님 1명을 위한 온라인 원생 관리 웹앱입니다. Python 3.12 이상만 있으면 실행할 수 있습니다. 외부 Python/JavaScript 패키지 설치는 필요하지 않습니다. Bootstrap 5.3.8 CSS는 프로젝트에 포함되어 서버에서 직접 제공합니다.
...
- 시험 카테고리 등록·이름 수정
- 100점 만점 성적, 총점만 입력 또는 객관식·서술형 나눠 입력, 선택 난이도, 카테고리별 날짜순 그래프
- 영역별 만점 기본값 70/30, 시험별 변경 가능(합계 100점), 영역 점수 합산 및 상세 표시
...
영역별 점수 기능 적용 후 서버를 재시작하고 브라우저를 새로고침하세요. 시작 시 기존 DB에 필요한 열을 자동 추가하며 기존 성적은 총점만 입력으로 유지합니다.
